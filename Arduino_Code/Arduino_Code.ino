...
const bool CALIBRATE_DRIVE = false;
 ...
 */
Sabertooth ST[2][4] = {{Sabertooth(128, SWSerial), Sabertooth(133, SWSerial), Sabertooth(129, SWSerial), Sabertooth(135, SWSerial)},//fourth element here is the conveyor!
                       {Sabertooth(130, SWSerial), Sabertooth(131, SWSerial), Sabertooth(132, SWSerial), Sabertooth(134, SWSerial)}};//there is no Sabertooth on address 134, it's just filler!
...
void runConveyorMotor(int motorNum, int vel) {
  if(ARDUINO_NUM == 1)
    ST[ARDUINO_NUM][3].motor(motorNum, vel);
}
